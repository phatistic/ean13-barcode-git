...
function ReadyToWriteTheEan13barcode (Ean13numTxt: string) {
    if (WriteNumberStrToEan(Ean13numTxt)) {
        SetEanEncode(LeanStr)
        BarcodeImage = DrawEanImage(60, 8, 1, scene.backgroundColor())
    }
}
...
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    if (WritingBarcode) {
        ReadyToWriteTheEan13barcode(EanInput)
        WritingBarcode = false
    } else {
        ReadyToWriteTheEan13barcode(GenerateNewEan13Number())
    }
})
...
function GenerateNewEan13Number () {
    t = ""
    for (let index = 0; index < randint(1, 12); index++) {
        t = "" + t + convertToText(randint(0, 9))
    }
    return t
}
